...
export default function CreateProject() {
  ...
  if (!user) {
    return <p className="max-w-lg mx-auto px-6 py-16 text-center text-ink/60 dark:text-ink-dark/60">Sign in to post a project.</p>;
  }
    ...
    <div className="max-w-lg mx-auto px-6 py-12">
      ...
      <form onSubmit={submit} className="space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <select value={stage} onChange={e => setStage(e.target.value)} className="p-2.5 rounded-lg border border-ink/15 dark:border-ink-dark/15 bg-surface dark:bg-surfacedark text-sm">
            {STAGES.map(s => <option key={s} value={s}>{s}</option>)}
          </select>
          <select value={type} onChange={e => setType(e.target.value)} className="p-2.5 rounded-lg border border-ink/15 dark:border-ink-dark/15 bg-surface dark:bg-surfacedark text-sm">
            <option value="">Type…</option>
            {TYPES.map(t => <option key={t} value={t}>{t.replace('_', ' ')}</option>)}
          </select>
          <select value={commitment} onChange={e => setCommitment(e.target.value)} className="p-2.5 rounded-lg border border-ink/15 dark:border-ink-dark/15 bg-surface dark:bg-surfacedark text-sm">
        ...
        <div>
          ...
          <div className="space-y-3 mt-2">
            {roles.map((r, i) => (
              <div key={i} className="p-3 rounded-lg border border-ink/10 dark:border-ink-dark/10 space-y-2">
                <div className="flex gap-2">
                  ...
                  <input
                    ...
                    className="w-20 p-2.5 rounded-lg border border-ink/15 dark:border-ink-dark/15 bg-surface dark:bg-surfacedark text-sm"
                  />
                  {roles.length > 1 && (
                ...
                <input
                  ...
                  className="w-full p-2 rounded-lg border border-ink/15 dark:border-ink-dark/15 bg-surface dark:bg-surfacedark text-sm"
                />
                <input
                ...
                />
                <div className="flex gap-2">
                  <select value={r.experience} onChange={e => updateRole(i, 'experience', e.target.value)} className="flex-1 p-2 rounded-lg border border-ink/15 dark:border-ink-dark/15 bg-surface dark:bg-surfacedark text-sm">
                    {EXPERIENCE.map(x => <option key={x} value={x}>{x}</option>)}
                  </select>
                  <select value={r.commitment} onChange={e => updateRole(i, 'commitment', e.target.value)} className="flex-1 p-2 rounded-lg border border-ink/15 dark:border-ink-dark/15 bg-surface dark:bg-surfacedark text-sm">
                    <option value="">Commitment…</option>
                    {COMMITMENTS.map(c => <option key={c} value={c}>{c}</option>)}
                  </select>
                </div>
