    ...
    <div className="max-w-5xl mx-auto px-6 py-10">
      ...
      <div className="flex gap-2 mb-6">
        <button
          ...
          className={`px-4 py-2 rounded-lg text-sm font-semibold ${tab === 'owned' ? 'bg-sky dark:bg-sky-dark text-white' : 'border border-ink/25 dark:border-ink-dark/25'}`}
        >
          Owned ({data.owned.length})
        </button>
        <button
          onClick={() => setTab('joined')}
          className={`px-4 py-2 rounded-lg text-sm font-semibold ${tab === 'joined' ? 'bg-sky dark:bg-sky-dark text-white' : 'border border-ink/25 dark:border-ink-dark/25'}`}
        >
