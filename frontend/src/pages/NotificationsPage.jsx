  ...
  return (
    <div className="max-w-2xl mx-auto px-6 py-10">
      <div className="flex items-center justify-between mb-6">
        ...
        {unreadCount > 0 && (
          <button onClick={markAllRead} className="text-sm font-semibold text-blue-text dark:text-blue-textdark">
            Mark all read
          </button>
        )}
      </div>
        ...
        <div className="rounded-xl border border-ink/20 dark:border-ink-dark/20 bg-surface dark:bg-surfacedark divide-y divide-ink/10 dark:divide-ink-dark/10">
          {notifications.map(n => (
            <button
              ...
              className={`w-full text-left p-4 text-sm hover:bg-page dark:hover:bg-pagedark ${!n.read ? 'bg-blue-soft/40 dark:bg-blue-softdark/40' : ''}`}
            >
              <p>{n.message}</p>
