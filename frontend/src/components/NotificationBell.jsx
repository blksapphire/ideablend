        ...
        <>
          ...
          <div className="absolute right-0 top-full mt-2 w-80 max-w-[90vw] bg-surface dark:bg-surfacedark border border-ink/20 dark:border-ink-dark/20 rounded-xl shadow-lg z-30 max-h-96 overflow-y-auto">
            <div className="flex items-center justify-between p-3 border-b border-ink/10 dark:border-ink-dark/10">
              ...
              {unreadCount > 0 && (
                <button onClick={markAllRead} className="text-xs text-blue-text dark:text-blue-textdark font-medium">
                  Mark all read
                </button>
              )}
            </div>
            {notifications.length === 0 ? (
              ...
              notifications.map(n => (
                <button
                  ...
                  className={`w-full text-left p-3 border-b border-ink/10 dark:border-ink-dark/10 last:border-none text-sm hover:bg-page dark:hover:bg-pagedark ${!n.read ? 'bg-blue-soft/40 dark:bg-blue-softdark/40' : ''}`}
                >
                  <p>{n.message}</p>
